---
date: 2016-08-31 24:00:00+00:00
layout: post
title: 微信平台之数据查询[更新了多条件查询]
catalog:    true
tags: 微信 数据 查询 开发
header-mask: 0.3
author:     "Hiralph"
    ...
    private function receiveText($object)
       {
		...
		$keyword= $object->Content;
		//$key='分数';    //这里设置查询方式的关键字，如我设置的是：分数
		//查询方式：分数班级@姓名,如：分数工商管理@小明
		preg_match('/分数(.*)/',$keyword, $match); 
		$cx = $match[1]; 
		$fuhao='@';//分隔符，可以写@或者+或者其他，将两个查询条件分隔开
	    $cxall=explode($fuhao,$cx);
	    $cx1=$cxall[0];//第一个条件，如专业：工商管理
		$cx2=$cxall[1];//第二个条件，如姓名：小明
		//$cx = substr($keyword,2,9); 

				$sql="select * from cx_score where zy='{$cx1}'  and name='{$cx2}' ";  //这里cx_score可以改成你自己的表名，zy,name改成你要设置的字段名,其他的不用改
				$result2 = mysql_query($sql);
				$row = mysql_fetch_assoc($result2);
...
#### 具体如下图
![chaxun6](/img/blog/cx-duo-new.jpg)
